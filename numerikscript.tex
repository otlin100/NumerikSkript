...
\begin{enumerate}
  ...
  \item[b)] Computertomographie \\
    ...
    Viele Strahlen: $L_j(t) = \omega ( \phi_j ) s_j + \omega^{\perp} ( \phi_j ) t$\\
    \includegraphics[width=8cm]{Bild2.png}\\
    $\omega(\phi_j) = (\cos(\phi_j), \sin(\phi_j))$\\
    $\omega^{\perp}(\phi_j) = (-\sin(\phi_j), \cos(\phi_j))$ \\\\
    $\begin{bmatrix} 
...
Sobald man die LR-Zerlegung (ln-decomposition) von $A$ kennt, löst man $Ax = b$ wie folgt:
...
%
und anschließend:\\
Löse $Rx = c$ wie in (15.2) angegeben ("Rückwärtssubsitution")
\end{nothing}
